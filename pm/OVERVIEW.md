...
Tarsi is an AI-powered changelog generator designed to streamline the process of creating user-friendly release notes for software projects. By analyzing recent git changes and referencing previous changelog entries, Tarsi leverages advanced language models (specifically Google's Gemini) to automatically draft new "Unreleased" sections in a clear, user-focused style. This CLI tool helps development teams maintain consistent, high-quality changelogs with minimal manual effort, ensuring that end users are always informed about the latest updates and improvements without exposing technical implementation details.
